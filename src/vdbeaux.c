  ...
  for(i=0; rc==SQLITE_OK && i<db->nDb; i++){ 
    ...
    if( sqlite3BtreeIsInTrans(pBt) ){
      ...
      ** journal modes use a master journal and which do not */
      ...
      if(pBt->pBt->pdbPager) {
	pdbPager = pBt->pBt->pdbPager;
	if( db->aDb[i].safety_level!=PAGER_SYNCHRONOUS_OFF
	    && aMJNeeded[pdb_sqlite3PagerGetJournalMode(pdbPager)]
	    && pdb_sqlite3PagerIsMemdb(pdbPager)==0
	    ){ 
	  assert( i!=1 );
	  nTrans++;
	}
	rc = pdb_sqlite3PagerExclusiveLock(pdbPager);
      } else {
...
int sqlite3VdbeFinalize(Vdbe *p){
  int rc = SQLITE_OK;
  if( p->magic==VDBE_MAGIC_RUN || p->magic==VDBE_MAGIC_HALT ){ // equals to halt
