...
static int readDbPage(PgHdr *pPg){
  ...
  /* printf(("FETCH %d page %d hash(%08x)\n", */
  /*              PAGERID(pPager), pPg->pgno, pager_pagehash(pPg))); */
...
static int getPageNormal(
  ...
  // vpm-anonymous's debug!!
  // printf("[PHEAPDEBUG] In getPageNormal! getting page no: %d, filesize: %d\n", pgno, n);
  if(pPager->fd->pMethods == NULL) {
      // printf("[PDEBUG] Error! Can't get pMethods here.\n");
  } else {
      // sqlite3OsFileSize(pPager->fd, &n);
  }
  // Debug end!
